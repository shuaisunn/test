...
Decrypt768 = function(c, privateKey) {
    ...
    var buffer1 = Buffer.from(m);
    var buffer2 = Buffer.from(pkh);
    var hash1 = new SHA3(512);
    ...
    if (!fail){
        ...
        var buffer4 = Buffer.from(kr1);
        var buffer5 = Buffer.from(ch);
...
function indcpaDecrypt(c, privateKey) {
    ...
    // extract ciphertext
    var u = decompress1(c.slice(0, 960));
    var v = decompress2(c.slice(960, 1088));
...
function generateMatrixA(seed, transposed) {
    ...
    for (var i = 0; i < paramsK; i++) {
        ...
        for (var j = 0; j < paramsK; j++) {
            ...
            xof.reset();
            var buffer1 = Buffer.from(seed);
            var buffer2 = Buffer.from(transpose);
...
function prf(l, key, nonce) {
    ...
    nonce_arr[0] = nonce;
    var hash = new SHAKE(256);
    hash.reset();
    var buffer1 = Buffer.from(key);
    var buffer2 = Buffer.from(nonce_arr);
...
Test768 = function(){
    ...
    var fs = require('fs');
    var textByLine = fs.readFileSync('./node_modules/crystals-kyber/PQCkemKAT_2400.rsp').toString().split("\n");
