...
Decrypt1024 = function(c, privateKey) {
    ...
    var sk = privateKey.slice(0, 1536); // indcpa secret key
    var pk = privateKey.slice(1536, 3104); // indcpa public key
    var pkh = privateKey.slice(3104, 3136); // sha3-256 hash
    var z = privateKey.slice(3136, 3168);
    ...
    var kr = new Uint8Array(hash1.digest());
    var kr1 = kr.slice(0, 32);
    var kr2 = kr.slice(32, 64);
...
function indcpaKeyGen() {
    ...
    const hash1 = new SHA3(512);
    hash1.update(buffer1);
    var seed = new Uint8Array(hash1.digest());
...
function indcpaDecrypt(c, privateKey) {
    ...
    // extract ciphertext
    var u = decompress1(c.slice(0, 1408));
    var v = decompress2(c.slice(1408, 1568));

    var privateKeyPolyvec = polyvecFromBytes(privateKey);
...
function generateMatrixA(seed, transposed) {
    ...
    for (var i = 0; i < paramsK; i++) {
        ...
        for (var j = 0; j < paramsK; j++) {
            ...
            var output = new Uint8Array(xof.digest({ buffer: Buffer.alloc(672)}));
...
function prf(l, key, nonce) {
    ...
    var buf = hash.digest({ buffer: Buffer.alloc(l)}); // 128 long byte array
    return buf;
}
...
Test1024 = function(){
    ...
    var fs = require('fs');
    var textByLine = fs.readFileSync('./node_modules/crystals-kyber/PQCkemKAT_3168.rsp').toString().split("\n");
    ...
    for (var i=0; i<100; i++){
        var ss2 = Decrypt1024(ct100[i],sk100[i]);
    ...
    var pk_sk = KeyGen1024();
    var pk = pk_sk[0];
    var sk = pk_sk[1];

    // To generate a random 256 bit symmetric key (ss) and its encapsulation (c)
    var c_ss = Encrypt1024(pk);
    var c = c_ss[0];
    var ss1 = c_ss[1];

    // To decapsulate and obtain the same symmetric key
    var ss2 = Decrypt1024(c, sk);
